...
def load_fits_data(fits_path: str, nside: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    Auto-detects available fields and fails fast on intensity-only products.

    Args:
        fits_path (str): Path to the input FITS file.
        nside (int): Target HEALPix NSIDE for analysis.

    Returns:
        Tuple[np.ndarray, np.ndarray]: (Q_down, U_down) at the requested NSIDE.

    Raises:
        ValueError: If the file does not contain polarization (Q/U) data.
    """
    ...
    # Read all available fields; returns shape (n_fields, npix) or (npix,) if single field
    m = hp.read_map(fits_path, field=None, verbose=False)
    ...
    # If single field → intensity-only (no Q/U)
    if getattr(m, "ndim", 1) == 1:
        raise ValueError(
            "Selected FITS contains one field (intensity-only, I). "
            "No polarization (Q/U) present. Use a polarization product "
            "(LFI 30/44/70 or HFI 100/143/217/353) — not 545/857."
        )

    # Multiple fields: pick Q and U robustly
    nfields = m.shape[0]
    if nfields >= 3:
        # Typical I,Q,U ordering
        stokes_q, stokes_u = m[1], m[2]
    elif nfields == 2:
        # Some products distribute Q/U only
        stokes_q, stokes_u = m[0], m[1]
    else:
        raise ValueError(
            f"Unexpected number of fields: {nfields}. Need at least Q and U."
        )

    # Replace NaNs before ud_grade to avoid propagating holes as spurious structure
    stokes_q = np.nan_to_num(np.asarray(stokes_q), nan=0.0)
    stokes_u = np.nan_to_num(np.asarray(stokes_u), nan=0.0)
